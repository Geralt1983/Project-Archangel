...
export default function BoardPage() {
  ...
  const handleBump = async (taskId: string, el: HTMLElement) => {
    ...
    // Stage 1: Elevate
    console.log("[v0] Stage 1: Adding bumpElevate class")
    el.classList.add("bumpElevate")
    await delay(140)

    // Stage 2: Wiggle
    console.log("[v0] Stage 2: Switching to bumpWiggle")
    el.classList.remove("bumpElevate")
    el.classList.add("bumpWiggle")
    await delay(240)

    // Stage 3: Fly
    console.log("[v0] Stage 3: Starting fly animation")
    ...
    if (doingAnchorRef.current) {
      console.log("[v0] Flying to doing column", { 
        ...
        target: doingAnchorRef.current.getBoundingClientRect() 
      })
      await animateFly(el, doingAnchorRef.current)
    } else {
      console.log("[v0] No doing anchor ref found!")
    }

    // Stage 4: Commit state change
    console.log("[v0] Stage 4: Committing state change")
